...
class RESISC45Dataset(VisionClassificationDataset):
    ...
    * https://doi.org/10.1109/jproc.2017.2675998
    ...
    def __init__(
        ...
        valid_fns = set()
        with open(os.path.join(self.root, "resisc", "resisc45", f"resisc45-{split}.txt")) as f:
            for fn in f:
        ...
        super().__init__(
            root=os.path.join(root, "resisc", self.directory),
...
class RESISC45:
    def __init__(self,
        ...
        self.test_dataset = RESISC45Dataset(root=location, split='test', transforms=preprocess)     #change back to test after - CHRISTOS
...
class RESISC45Val(RESISC45):
    def __init__(self,
        ...
        self.test_dataset = RESISC45Dataset(root=location, split='val', transforms=preprocess)
        self.test_loader = torch.utils.data.DataLoader(
            self.test_dataset,
            batch_size=batch_size,
            num_workers=num_workers
